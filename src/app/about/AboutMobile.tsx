...
export default function AboutMobile() {
  ...
  }, [shouldLoadRest, preloaderDone, updateMetrics]);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const panels = scopeRef.current?.querySelectorAll<HTMLElement>(".about-stack-layer");
    ...
    const handleScroll = (e?: any) => {
      ...
      // Absolute position of the track top relative to current page top
      const trackTopDoc = rect.top + scrollY;
      
      // Calculate scroll distance within track
      const relativeScroll = clamp(scrollY - trackTopDoc, 0, totalScrollable);

      // PINNING MECHANISM: Move frame down at identical rate of scroll
      fixedFrameRef.current.style.transform = `translate3d(0, ${relativeScroll}px, 0)`;

      // Set current animation progress state (0 to 1)
      rawProgress.current = relativeScroll / totalScrollable;
  ...
  }, [shouldLoadRest, preloaderDone, smootherRef, triggerSec5Hook]);

  return (
    <div ref={scopeRef} className="w-full">
      {/* Scroll track container providing total 700vh scroll height */}
      <div
        ref={trackRef}
        className="about-track-container relative w-full"
        style={{ height: "700vh" }}
      >
        {/* Absolute frame pinned continuously via JS translate3d */}
        <div
          ref={fixedFrameRef}
          className="absolute top-0 left-0 w-full overflow-hidden z-10 h-[100dvh] will-change-transform"
        >
